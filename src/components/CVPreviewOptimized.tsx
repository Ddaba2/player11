...
import { IdentityPill, MetricBox, Section, StatRow } from './CVComponents';
...
const CVPreviewOptimized = memo(function CVPreviewOptimized({ cv, template = 'modern' }: CVPreviewProps) {
  ...
  const getSportEmoji = useCallback((sport: string) => {
                    ...
                    <div key={entry.id} style={{ display: 'flex', gap: '16px' }}>
                      <div
                        style={{
                          ...
                          height: '40px',
                          borderRadius: '8px',
                          backgroundColor: templateStyles.metrics.backgroundColor,
                      ...
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        ...
                        <div style={{ fontSize: '11px', color: '#64748b', lineHeight: 1.4 }}>
                          {formatDateRange(entry.start_year, entry.end_year)}
                        </div>
                        {entry.description ? (
...
// Helper functions
function dominantShort(side: string): string {
...
function getPositionProfile(position: string): 'defender' | 'midfielder' | 'attacker' {
...
function formatDateRange(startYear: string, endYear: string): string {
  if (!startYear && !endYear) return '—';
  if (!startYear) return endYear;
  if (!endYear) return startYear;
  
  // Gérer "présent" ou "present"
  if (/présent|present|actuel|en\s*cours/i.test(endYear)) {
...
export default CVPreviewOptimized;
